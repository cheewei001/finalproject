...
class UserManagerDB
{
    public static function fillUser($row){
		...
		$user->wants_news = $row["wants_news"];
        return $user;
    }
    ...
    public static function saveUser(User $user){
        ...
        $stmt->bind_param("issssssi", $user->id,$user->firstname, $user->lastname, $user->email,$user->password, $user->account_creation_time, $user->role, $user->wants_news); 
        $stmt->execute();
        if($stmt->errno!=0){
            printf("Error: %s.\n",$stmt->error);
        }
...
// 			echo "Record updated successfully";
    ...
    public static function unsubscribenews($id){
        $conn=DBUtil::getConnection();
        $sql="UPDATE tb_user SET wants_news = FALSE WHERE id ='$id';";
        $stmt = $conn->prepare($sql);
        if ($conn->query($sql) === TRUE) {
//         echo "Record updated successfully";
        } else {
